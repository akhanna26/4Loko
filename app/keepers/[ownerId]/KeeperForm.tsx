...
export default function KeeperForm({
  ...
  return (
    <div className="border-2 shadow-sm overflow-hidden"
      ...
      <div className="p-1">
        {sortedRoster.map((g) => {
          ...
          return (
            <label
              key={g.golfer_id}
              className="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 cursor-pointer transition-all hover:shadow-md"
              style={{
                background: isSelected
                  ? `linear-gradient(135deg, ${theme.primary}20 0%, rgba(255, 255, 255, 0.98) 50%, ${theme.primary}20 100%)`
                  : 'white',
                margin: '6px',
      ...
      <div className="px-4 sm:px-5 py-3 sm:py-4 border-t border-[color:var(--green-forest)]/15 flex items-center gap-3 justify-end flex-wrap" style={{ background: `${theme.primary}08` }}>
        {currentDeclaration && (
          <button
            onClick={handleClear}
            disabled={isPending}
